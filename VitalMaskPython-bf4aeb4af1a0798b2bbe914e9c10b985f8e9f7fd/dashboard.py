...
BLE_DEBUG = True
BLE_DICT = {
    'E0:9B:25:9C:EB:07' : 'Vita 1',
    'D1:F0:30:37:ED:8F' : 'Vita 2',
    'D3:5F:73:8C:FA:E1' : 'Vita 3',
    'C8:A1:F0:FA:3C:4E' : 'Vita 4',
    'DB:96:BA:10:09:68' : 'Vita 5',
    'C1:AF:92:E5:62:FA' : 'Vita 6',
    'EE:A8:3D:B8:7B:7D' : 'Vita 7',
    '00:SI:MU:LA:TE:00' : '00:SI:MU:LA:TE:00'
    }
INV_BLE_DICT = {v: k for k, v in BLE_DICT.items()}
...
wakeup = SoundLoader.load('mp3/solemn-522.mp3')
wakeup.play()

#alarmStr = 'mp3/appointed-529.mp3'
#alarmStr = 'mp3/done-for-you-612.mp3'
...
TEMPERATURE_CHAR_UUID = "00001402-0000-1000-8000-00805f9b34fb"
# Rishi's test UUID
...
class SaveDialog(FloatLayout):
    def __init__(self, **kwargs):
        ...
        dir = Config.get('saving', 'dir')
        if dir == "":
            dir = os.getcwd()
        print(f'TESTING: {dir}')
        self.ids.filechooser.path = dir
        self.ids.text_input.text = dir # Config.get('saving', 'dir')
...
class Mask():
    ...
    def __init__(self, addr, num, pui, avpu, age, sex, cur):
        ...
        self.delta_spo2 = 0
        self.delta_rr = 0
        # 2 different scoring methods, changeable in settings
        self.priority = 0
        self.news = 0
    ...
    def update(self):
        ...
        if (sound_alarm) and vitals_exceeded != 0:
            # TODO: modifying pitch is leading to being unable to play audio file
    ...
    # Write data to using writer
    def writeData(self, writer):
        time = datetime.now().strftime("%Y_%m_%d_%H_%M_%S")
        row = [time, self.num, self.age, self.sex, self.spo2_val, self.hr_val, self.rr_val, self.temp_val]
        writer.writerow(row)
    ...
    def get_news(self):
        ...
        if self.hr_val == -1:
            pass
        elif self.hr_val <= 40 or self.hr_val >= 131:
            news += 3
        ...
        if self.rr_val == -1:
            pass
        elif self.rr_val <= 8 or self.rr_val >= 25:
            news += 3
        elif self.rr_val > 20:
            news += 2
        elif self.rr_val <= 11:
...
class DashboardWindow(BoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        # Set default path for filechooser
        print(self.ids)
        #self.ids.filechooser.path = 'C:\\Users\\Daniel\\Documents\\VitalMaskPython' #Config.get('saving', 'dir')
        self.simulate_queue = ['8', '9', '10', '11', '12', '13', '14', '15']
        if(DATABASE_DEBUG):
            try:
    ...
    def createCSV(self):
        ...
        # Create csv writer
        dir = Config.get('saving', 'dir') # Path only returns C:\
        dir += '/' # Dir doesn't include last slash
        filename = Config.get('saving', 'filename')
        ...
        self.file = open(f'{dir + filename}.csv', 'w', newline='')
        print(f'Data Filename = {self.file}')
        self.writer = csv.writer(self.file)
        self.writer.writerow(['Time', 'Mask ID', 'Age', 'Sex', 'SpO2', 'HR', 'RR', 'Temp'])
    ...
    def add_mask(self):
        ...
        mask_addr = INV_BLE_DICT[self.ids.addr_spinner.text]
        # mask_num = self.ids.mask_num_inp.text
        if self.ids.addr_spinner.text == "00:SI:MU:LA:TE:00":
            mask_num = self.simulate_queue.pop(0)
        else:
            mask_num = self.ids.addr_spinner.text[5]
        ...
        if mask_num.isnumeric() and mask_num not in mask_dict and len(mask_addr.replace(':', '')) == 12 and mask_age is not None:
            ...
            new_mask = Mask(mask_addr, mask_num, mask_pui, mask_avpu,
    ...
    def update(self, update_mask):
        ...
        if(self.sort_by_id):
            sorted_masks = sorted(mask_dict.values(),
                                  key=lambda mask: int(mask.num), reverse=True)
            ...
            if(scoring_method == "NEWS"):
                ...
                sorted_masks = sorted(mask_dict.values(),
                                      key=lambda mask: (mask.priority, -int(mask.num)))
        else:
            sorted_masks = sorted(mask_dict.values(),
        ...
        if scoring_method == "NEWS":
            self.ids.sort_priority_button.text = 'Score'
        else:
            self.ids.sort_priority_button.text = 'Priority'
    ...
    async def discover(self):
        ...
        addr_list = (BLE_DICT[device.address] for device in devices if "Vita" in device.name)
        simulate_addr = ["00:SI:MU:LA:TE:00"]
        self.ids.addr_spinner.values = addr_list
        # TODO: Remove simulate_addr from next line when releasing
        self.ids.addr_spinner.values = self.ids.addr_spinner.values + simulate_addr
        if len(self.ids.addr_spinner.values) == 0:
            self.ids.addr_spinner.text = "No Devices Found"
        else:
...
class DashboardApp(App):
    ...
    def show_save(self):
        ...
        self._popup = Popup(title="Save Directory/File", content=content,
                            size_hint=(0.9, 0.9), size=(400, 400))
        self._popup.open()
        
    def save(self, path, filename):
        print(os.path.isdir(filename))
        if os.path.isdir(filename): # dir doesn't include last slash
            filename += '\\'
        head, tail = os.path.split(filename)
        print('In save')
        print(f'Path: {head}')
        print(f'Filename: {tail}')
        Config.set('saving', 'dir', head)
        Config.set('saving', 'filename', tail)
        Config.write()
